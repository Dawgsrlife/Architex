      ...
      <Link href={`/dashboard/${project.id}`}>
        <div className="relative bg-white border border-stone-200 rounded-xl p-6 hover:border-stone-300 hover:shadow-sm transition-all duration-200">
          ...
          <div className="flex items-center justify-between text-xs text-stone-400">
            <div className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{project.updatedAt}</div>
            <div className="flex items-center gap-1"><Grid3X3 className="w-3.5 h-3.5" />{project.nodesCount} nodes</div>
          </div>
        ...
        {menuOpen && (
          <>
            ...
            <div className="absolute right-0 top-full mt-1 w-40 bg-white border border-stone-200 rounded-lg shadow-lg z-50 overflow-hidden">
              <button className="w-full flex items-center gap-2 px-3 py-2 text-sm text-stone-600 hover:bg-stone-50 transition-colors"><ExternalLink className="w-4 h-4" />Open</button>
              ...
              <button className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors"><Trash2 className="w-4 h-4" />Delete</button>
            </div>
          </>
        )}
      </div>
...
export default function DashboardPage() {
  ...
  const stats = { total: mockProjects.length, active: mockProjects.filter(p => p.status === "active").length, draft: mockProjects.filter(p => p.status === "draft").length };
      ...
      <main className="pt-24 pb-16 px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          ...
          {filteredProjects.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{filteredProjects.map((project, index) => <ProjectCard key={project.id} project={project} index={index} />)}</div>
          ) : (
              ...
              {filteredProjects.map((project) => (
                <Link key={project.id} href={`/dashboard/${project.id}`} className="block">
                  <div className="flex items-center gap-4 p-4 bg-white border border-stone-200 rounded-xl hover:border-stone-300 hover:shadow-sm transition-all group">
                    ...
                    <div className="hidden sm:flex items-center gap-6 text-xs text-stone-400"><div className="flex items-center gap-1"><Grid3X3 className="w-3.5 h-3.5" />{project.nodesCount}</div><div className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{project.updatedAt}</div></div>
