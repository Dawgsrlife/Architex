  ...
  return (
    <>
      ...
      <div className="fixed z-50 bg-white border border-stone-200 rounded-xl shadow-lg py-1 min-w-[160px]" style={{ left: x, top: y }}>
        ...
        <button onClick={() => { onUndo(); onClose(); }} disabled={!canUndo} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-stone-600 hover:bg-stone-50 transition-colors disabled:opacity-40 disabled:hover:bg-transparent"><Undo2 className="w-4 h-4" />Undo<span className="ml-auto text-xs text-stone-400">Ctrl+Z</span></button>
        <button onClick={() => { onRedo(); onClose(); }} disabled={!canRedo} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-stone-600 hover:bg-stone-50 transition-colors disabled:opacity-40 disabled:hover:bg-transparent"><Redo2 className="w-4 h-4" />Redo<span className="ml-auto text-xs text-stone-400">Ctrl+Y</span></button>
        <div className="h-px bg-stone-100 my-1" />
...
export default function ArchitectureCanvas() {
  const { nodes, edges, onNodesChange, onEdgesChange, onConnect, addNode, deleteNode, undo, redo, canUndo, canRedo, initHistory } = useArchitectureStore();
